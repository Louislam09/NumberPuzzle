...
class Itemize {
    ...
    if (this.childIsItemizable(child, parent)) {
      ...
      if (parent.itemizeOptions.dragAndDrop) {
        ...
        child.addEventListener("dragend", event => {
          ...
          event.stopPropagation();
          if (this.draggedEl) {
            if (this.draggedEl.opacityBeforeDrag) {
              this.draggedEl.style.opacity = this.draggedEl.opacityBeforeDrag;
            }
            this.draggedEl = null;
          }
          return true;
        ...
        child.addEventListener("dragover", event => {
          event.preventDefault();
          if (this.draggedEl && !child.inFlipAnim) {
            if (
              this.draggedEl.parentNode === child.parentNode &&
              !this.draggedEl.inFlipAnim
            ) {
              this.draggedEl.style.opacity = "0.6";
              this.flipRead(this.items);
              let draggedElNodeIndex = Array.prototype.indexOf.call(
                this.draggedEl.parentNode.children,
                this.draggedEl
              );
              ...
              if (draggedElNodeIndex < draggedOnNodeIndex) {
                if (
                  ...
                  this.draggedEl.parentNode.insertBefore(
                    this.draggedEl,
                    child.nextElementSibling
                  );
                } else if (
                  ...
                  this.draggedEl.parentNode.insertBefore(temp, child);
                  this.draggedEl.parentNode.insertBefore(child, temp);
                  this.draggedEl.parentNode.insertBefore(this.draggedEl, temp);
                  this.draggedEl.parentNode.removeChild(temp);
                }
              } else if (draggedElNodeIndex > draggedOnNodeIndex) {
                this.draggedEl.parentNode.insertBefore(this.draggedEl, child);
              }
              this.flipPlay(this.items, 250, true);
            }
          }
          return false;
        });
        ...
        child.addEventListener("drop", event => {
          event.preventDefault();
          if (this.draggedEl && this.draggedEl.opacityBeforeDrag) {
            this.draggedEl.style.opacity = this.draggedEl.opacityBeforeDrag;
          }
          return false;
        });
  ...
  flipPlay(elems, duration, dragAnim) {
